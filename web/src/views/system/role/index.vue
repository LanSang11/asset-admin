...
async function updateRoleAuthorized() {
  ...
  try {
    const { withStepUp } = await import('@/composables/useStepUp')
    const res = await withStepUp('role_authorize', (headers) =>
      api.updateRoleAuthorized(
        {
          id: role_id.value,
