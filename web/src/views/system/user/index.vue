...
const {
  ...
  name: '用户',
  initForm: {},
  doCreate: (data) => withStepUp('user_create', (headers) => api.createUser(data, headers)),
  doUpdate: (data) =>
    withStepUp('user_update_security', (headers) => api.updateUser(data, headers)),
  doDelete: api.deleteUser,
  stepUpDeleteKey: 'user_delete',
        ...
        !row.is_superuser && h(
          NPopconfirm,
          {
            onPositiveClick: async () => {
              try {
                await withStepUp('user_totp_reset', (headers) =>
                  api.resetUserTotp({ user_id: row.id }, headers)
                )
          ...
          },
          {
            trigger: () =>
              h(
                ...
                {
                  default: () => '重置动态验证器',
                  icon: renderIcon('material-symbols:security', { size: 16 }),
                }
              ),
            default: () =>
              h(
                'div',
                {},
                '确定重置该用户的动态验证器和安全问题吗？该用户旧会话将立即失效，下次登录必须重新绑定。'
              ),
          }
        ),
        !row.is_superuser && h(
          NPopconfirm,
          {
            onPositiveClick: async () => {
              try {
                const res = await withStepUp('user_reset_password', (headers) =>
                  api.resetPassword({ user_id: row.id }, headers)
                )
                const tempPwd = res?.data?.temporary_password;
                // 修复：一次性密码经接口返回并弹窗展示（原提示"查看服务日志"，但日志已按安全要求只留掩码）
                dialog.success({
                  title: '密码已重置',
                  content: tempPwd
                    ? `该用户的一次性密码（仅显示一次，请立即转交用户；用户首次登录后必须修改）：\n\n${tempPwd}`
                    : '密码已重置（一次性密码获取失败，请查看服务日志）',
                  positiveText: '我已复制',
                });
                await $table.value?.handleSearch();
              } catch (error) {
...
async function handleUpdateDisable(row) {
  ...
  try {
    await withStepUp('user_update_security', (headers) => api.updateUser(row, headers))
