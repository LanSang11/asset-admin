...
test('登录第二步：密码通过后出 6 位框，拦截器不弹红字失败', () => {
  ...
  const backend = read('app/api/v1/base/base.py')
  const challenge = read('app/core/login_totp_challenge.py')
  assert.match(login, /请输入验证器中的 6 位动态码/)
  assert.match(login, /next_step === 'totp'/)
  assert.match(login, /login_challenge/)
  assert.match(login, /enterTotpStep/)
  assert.match(login, /stopVerifyMsg/)
  assert.match(login, /登录未完成，请重试/)
  assert.match(login, /验证并登录/)
  assert.match(login, /返回重新登录/)
  assert.doesNotMatch(login, /并重新完成滑块/)
  assert.doesNotMatch(login, /watch\(/)
  ...
  assert.match(backend, /login_totp_challenge/)
  assert.match(backend, /challenge_payload/)
  assert.match(backend, /密码已通过，请输入验证器 6 位动态码/)
  assert.match(challenge, /"next_step": "totp"/)
...
test('AI 预设默认 deepseek-v4-flash，选厂商只补模型与地址', () => {
  ...
  assert.doesNotMatch(presets, new RegExp("sk" + "-[a-zA-Z0-9]{8,}"))
...
test('教程写清超管必须绑验证器，不教 SQL 关闭', () => {
  ...
  assert.match(guide, /临时免登录动态码/)
  assert.match(guide, /限时验收模式/)
  assert.match(guide, /15～480/)
